...
from src.experiment.eval import evaluate
...
def main():
    ...
    match args.pipe.lower():
        ...
        case 'evaluate':
            metrics = ['p@1', 'mrr'] # Can modify desired metrics here. Reference Ranx library to get list of valid metric names.
            if not (args.c_input_path and args.test_split_path and args.corrupted):
                raise argparse.ArgumentError(None, f"Missing required arguments for evaluation pipe. Usage: --pipe evaluation -c <Long-CLIP checkpoint path> -t <test_split_path> -x <corrupted_files_path> -o <checkpoint_output_path> [--distributed]")
            evaluate(args.c_input_path, args.test_split_path, args.corrupted, metrics, args.return_mean, args.eval_output, args.dataset_divides)
