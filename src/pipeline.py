...
def get_args():
    ...
    data_parser.add_argument('--has_header', action="store_true", help="If your dataset has a header, this flag will make sure to skip it to prevent errors.")
    ...
    data_parser.add_argument('--remove_corrupted', '-r', action="store_true", help="Remove corrupted samples from dataset if flag provided.")
    ...
    evaluate_parser.add_argument('--test_split_path', '-t', type=str, help="Path to test split .tsv file (generated by data pipe).")
    ...
    match args.pipe_type.lower():
        case 'data':
            ...
            match args.dataset_type.lower():
                case 'generic':
                    dataset, missing = process_generic(args.metadata_path, validate_data=True, has_header=args.has_header, remove_corrupted=args.remove_corrupted)
                case 'mse':
                    dataset, missing = process_mse(args.metadata_path, args.images_path, validate_data=True, remove_corrupted=args.remove_corrupted)
                case 'wiki':
                    dataset, missing = process_mse(args.metadata_path, args.images_path, validate_data=True, remove_corrupted=args.remove_corrupted)
